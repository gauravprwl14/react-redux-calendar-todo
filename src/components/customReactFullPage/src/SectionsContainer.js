...
export default class SectionsContainer extends React.Component {
  ...
  _addActiveClass() {
    ...
    let parentNode = document.getElementById("trainHeaderContainer");

    if (parentNode) {
      parentNode.classList.add(hash + "-active-parent");
    }
  }
  ...
  _removeActiveClass() {
    ...
    if (parentNode) {
      const parentClassNameArr = [...parentNode.classList];
      const index = parentClassNameArr.find(classNameInString => {
        if (classNameInString.includes("-active-parent")) {
          return true;
        }
        return false;
      });
      if (index) {
        parentNode.classList.remove(index);
      }
    }
  }
